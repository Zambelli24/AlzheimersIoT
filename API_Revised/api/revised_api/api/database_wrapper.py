    ...
    def get_data_since(self, key, checked_time):
        ...
        for data_piece in db.find():
            if data_piece['key'] == key:
                data_time = iso8601.parse_date(data_piece['time'])
                if data_time > checked_time and data_time < current_time:
                    return_dict[data_piece['time']] = json.dumps(data_piece['data'])
        return return_dict
        ...
        for data_piece in db.find():
            if data_piece['key'] == key:
                ...
                if return_value == {} or data_time > time:
                    return_value.clear()
                    time = data_time
                    return_value[data_piece['time']] = json.dumps(data_piece['data'])
        return return_value
